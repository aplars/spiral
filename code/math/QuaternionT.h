...
namespace sa
{
	...
	public:
		...
		QuaternionT(const Vector3T<T>& v, const T& s) 
			: m_s(s)
			, m_v(v) {}
		...
		void LoadFromAxisAngle(const Vector3T<T> &v, const T& angleRad)
		{
			T sinAngle;
			angleRad *= 0.5f;
			Vector3T<T> vn(v);
		...
		static QuaternionT GetFromAxisAngle(const Vector3T<T> &v, const T& angleRad)
		{
			QuaternionT<T> ret;
      ...
      Normalize();
		...
		const Vector3T<T> operator*(const Vector3T<T> &vec) const
		{
			Vector3T<T> vn(vec);
			vn.normalise();
      ...
      return (Vector3T<T>(resQuat.x, resQuat.y, resQuat.z));
		}
	...
	/*	Vector4T<T> Mult(const math::Vector4T<T>& v) const
		...
		Vector3T<T> Vec3Transform(const Vector3T<T>& in) const
		{
			return *this*in;
		}
		...
		static  QuaternionT<T> SLerp(const T& pFactor, QuaternionT<T>& pStart, QuaternionT<T>& pEnd)
		{
			...
			{
				cosom = -cosom;
				end.m_v.xyz[0] = -end.m_v.X();   // Reverse all signs
				end.m_v.xyz[1] = -end.m_v.Y();
				end.m_v.xyz[2] = -end.m_v.Z();
				end.m_s = -end.m_s;
			} 
			...
			pOut.m_v.xyz[0] = sclp * pStart.m_v.X() + sclq * end.m_v.X();
			pOut.m_v.xyz[1] = sclp * pStart.m_v.Y() + sclq * end.m_v.Y();
			pOut.m_v.xyz[2] = sclp * pStart.m_v.Z() + sclq * end.m_v.Z();
