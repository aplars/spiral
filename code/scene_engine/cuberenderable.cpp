...
void CubeRenderable::toGPU(const ConfigurationManager& config, RenderDevice *device, RenderContext *context)
{
  ...
  m_drawData.Matrix4Uniforms[sp->uniformLocation("u_modelMatrix")] = Matrix44T<float>::GetTranslate(m_position);
  m_drawData.Vec4Uniforms[sp->uniformLocation("u_color")] = Vector4T<float>(1,0,0,0.5);
}
